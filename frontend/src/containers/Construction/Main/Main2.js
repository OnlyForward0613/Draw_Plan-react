...
import { Modal, Select, Input, FloatButton, Spin, ConfigProvider, theme } from "antd";
import { LogoutOutlined, PlusOutlined, ZoomInOutlined, ZoomOutOutlined } from '@ant-design/icons';
...
const App = (props) => {
  ...
  const onOkSetScaleModal = () => {
    const categorylist = props.data && props.data.map((item) => {return item.name});
            ...
            switch(item.type) {
              case "polygon":
                const DeductToDelete = [
                  ...
                  { x: lastState.x + lastState.width, y: lastState.y + lastState.height },
                ...
                if(isPolygonInsidePolygon(DeductToDelete, item.location)) {
                  ...
                  let calWidth = (Math.abs(lastState.width) * layoutSize / canvasWidth) * pageScale / 1000;
                  let calHeight = (Math.abs(lastState.height) * layoutSize / canvasWidth) * pageScale / 1000;
                  newWidget[index].deductRect = [...prevDeductRects];
                  newWidget[index].measure = (parseFloat(item.measure) + parseFloat((calWidth * calHeight).toFixed(2))).toFixed(2);
                  let temp = item.price.map(element => {
                    return (parseFloat((parseFloat(element) / parseFloat(measured_value)).toFixed(2)) * (parseFloat(parseFloat(measured_value) + (calWidth * calHeight)).toFixed(2))).toFixed(2)
                  } );
                ...
                break;
              case "rect":
                const previousRect = { x: item.location[0].x, y: item.location[0].y, width: item.location[1].x - item.location[0].x, height: item.location[1].y - item.location[0].y}
                if(isUserRectangleInsidePrevious(lastState, previousRect)) {
                  ...
                  let temp = item.price.map(element => {
                    return (parseFloat((parseFloat(element) / parseFloat(measured_value)).toFixed(2)) * (parseFloat(parseFloat(measured_value) + parseFloat((calWidth * calHeight).toFixed(2))).toFixed(2))).toFixed(2);
                  } );
                  newWidget[index].price = temp;
...
const handleSingleClick = (e) => {
  ...
  if(drawType === "dot") {
    ...
    setPrevState([
      ...prevState,
      {id: id, type: "dot", x: getMouseClickPosition(drawcanvas, e).x, y: getMouseClickPosition(drawcanvas, e).y}
    ])
    ...
    if(dotArray.length) {
      ...
      setWidgetData([
        ...widgetData,
        {
          ...
          measure: dotNum+1,
          result: dotNum+1,
          price: parseFloat(props.data.filter(item => item.name === labelcategory)[0].price) * (dotNum+1),
        }
      ])
    }
  }
...
function isUserRectangleInsidePrevious(userRect, previousRect) {
  ...
  const userBottomRight = { x: userRect.x + userRect.width, y: userRect.y + userRect.height };
...
function isPolygonInsidePolygon(polygon, previousPolygon) {
  ...
  const polygonVectors = polygon.map(vertex => ({ x: vertex.x, y: vertex.y }));
  const previousPolygonVectors = previousPolygon.map(vertex => ({ x: vertex.x, y: vertex.y }));
...
const handleDoubleClick = (e) => {
  ...
  switch(drawType) {
    ...
    case "rect":
      ...
      setPrevState([
        ...prevState,
        {id: id, type: "rect", x: startPoint.x, y: startPoint.y, height: y - startPoint.y, width: x - startPoint.x}
      ])
      ...
      widgetData.map((item, index) => {
        switch(item.type) {
          case "polygon":
            ...
            let isInside_Poly = isPolygonInsidePolygon(userPolygonVertices, item.location);

            if (isInside_Poly) {
              
              if(item.deductRect.length === 0){
                context.clearRect(startPoint.x, startPoint.y, x - startPoint.x, y - startPoint.y);
              }
              else {
                item.deductRect.forEach(element => context.clearRect(element.x, element.y, element.x1 - element.x, element.y1 - element.y));
              }
              
              setPrevState([
                ...prevState,
                {id: item.id, type: "deduct", x: startPoint.x, y: startPoint.y, height: y - startPoint.y, width: x - startPoint.x}
              ])

              const newArray = [...widgetData];
              let measured_value = item.measure;
              newArray[index].deductRect = [...item.deductRect, {x: startPoint.x, y:startPoint.y, x1: x, y1: y}];
              newArray[index].measure = (item.measure - displayValue).toFixed(2);              

              let temp = item.price.map(element => {
                 return ((parseFloat(element) / measured_value) * (measured_value - displayValue)).toFixed(2);
              });
          ...
          case "rect":
            ...
            const deductRect = { x:  startPoint.x, y: startPoint.y, width: x - startPoint.x, height: y - startPoint.y };

            const isInside = isUserRectangleInsidePrevious(deductRect, previousRect);
            ...
            if(isInside){
              ...
              newArray[index].deductRect = [...item.deductRect, {x: startPoint.x, y:startPoint.y, x1: x, y1: y}];
              newArray[index].measure = parseFloat((item.measure - displayValue).toFixed(2));             

              let temp = item.price.map(element => parseFloat(((parseFloat(element) / item.measure) * (item.measure - displayValue)).toFixed(2)));
...
const redraw = () => {
  ...
  prevState && prevState.map(item => {
    ...
    if(item.type === "line") {
      ...
      context.fillStyle = "rgba(255, 0, 0, 0.7)";
      selectedID && selectedID.forEach(element => {
        ...
        
      {
        file && (
          <div>
            ...
            >
              <Spin spinning={isLoading} fullscreen  size = "large" style={{width: "100%", height:"1000px"}}> 
                <div className="row">
                      ...
                      >
                        <div id="listpage">
                          {Array.from(Array(numPages), (e, i) => (
                            <div className={(pageNumber === i + 1) ? "perpage loadedpage" : "perpage"} onClick={() => {setPageNumber(i + 1)}}>
                  ...
                  <div className="col-md-10 page">
                    ...
                    <DraggableWidget widgetData={widgetData} onDelete={deleteSeletedData} selectData={DataSelect} />
                    {
